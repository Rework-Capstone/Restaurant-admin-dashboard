...
const FoodOrders = () => {
  ...
  return (
    <React.Fragment>
      ...
      <Content>
        <BlockHead size="lg">
            <div className="nk-block-head-sub"><span>Manage Food Orders</span></div>
            <BlockBetween size="md" className="g-4">
                <BlockHeadContent>
                <BlockTitle tag="h2" className="fw-normal">My Food Order</BlockTitle>
                <BlockDes>
                    <p>Here is the list of food orders that you have. <span className="text-primary"><Icon name="info" /></span></p>
                </BlockDes>
                </BlockHeadContent>
                <BlockHeadContent>
            ...
            <Card key={index} className="card-bordered sp-plan">
                ...
                <Col md="8">
                        ...
                        <Row className="gx-0 gy-3">
                            <Col xl="9" sm="8">
                                <div className="sp-plan-name">
                                    <h6 className="title"><Link to={`${process.env.PUBLIC_URL}/subscription/subscription-details/${item.id}`}> Debby Order <Badge color={item.status == 'Active' ? 'success' : 'light'} className="rounded-pill">{item.status}</Badge></Link></h6>
                                    <p>Food ID: <span className="text-base">{item.id}</span></p>
                                </div>
                            </Col>
                            <Col xl="3" sm="4">
                    ...
                    <div className="sp-plan-desc card-inner">
                        <Row className="gx-1">
                            <Col col="6" lg="3">
                                <p><span className="text-soft">Food name</span> {item.startdate}</p>
                            </Col>
                            <Col col="6" lg="3">
                                <p><span className="text-soft">Category</span> {item.recurring ? 'Yes' : 'No'}</p>
                            </Col>
                            <Col col="6" lg="3">
                                <p><span className="text-soft">Price</span> {item.price}</p>
                            </Col>
                            <Col col="6" lg="3">
                                <p><span className="text-soft">Status</span> {item.access}</p>
                            </Col> 
                        </Row>
                    </div>
                </Col>
